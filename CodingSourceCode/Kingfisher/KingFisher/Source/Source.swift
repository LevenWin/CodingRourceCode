...
/// - provider: The target image should be provided in data format.Normally, it can be an image from local storage or in any other encoding format (like Base64).
public enum Source {
    
    /// Represents the source task identifier when setting an image to view with extension methods
    ...
    /// The target image should be provided in a data format. Normally it can be an image form local storage or in any other encoding format(like Base64).
    case provider(ImageDataProvider)
    
    // MARK: Geting Properties
